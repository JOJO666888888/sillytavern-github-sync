...
function gitWithEnv(repoDir) {
    return simpleGit(repoDir || undefined).env(GIT_ENV);
}
...
async function cloneRepo(config, targetDir) {
    ...
    try {
        ...
        await git.clone(remoteUrl, targetDir);
        const repoSg = gitWithEnv(targetDir);
...
async function pullRepo(config, repoDir) {
    ...
    const pullResult = await git.pull('origin', config.branch, ['-X', 'theirs']);
    return {
