...
GitHub Token 的读取优先级：环境变量 `ST_GITHUB_SYNC_TOKEN` → 密钥文件 → 旧的配置字段。

- 密钥文件位于 `~/.sillytavern-github-sync/<用户>.token`，权限 `0600`，**在数据目录之外** ——
  不会被同步、备份或数据快照带走
- 首次启动会把配置里的明文 Token **自动迁移**到密钥文件并清空原字段，**无需任何手动操作**
- Token **绝不会**以明文形式发送到浏览器；所有 git 错误信息中的 Token 都会先脱敏
- Token 不会写入 `.git/config`：remote URL 始终是干净的 `https://github.com/owner/repo.git`，
  凭据通过 `GIT_ASKPASS` 助手传递，也不会出现在命令行参数里
- 同步仓库本地副本位于 `data/default-user/.github-data-sync/`

> **注意：** 密钥文件本身仍是明文，只是位置（数据目录之外）和权限（0600）都更安全。
> 如果部署环境支持注入环境变量，优先使用 `ST_GITHUB_SYNC_TOKEN`，它优先级最高。
...
| 插件未自动更新 | 检查 `config.yaml` 中是否设置了 `enableServerPluginsAutoUpdate: true`（仅对服务端插件生效） |
| 提示"拉取前备份失败，已中止" | 这是保护机制：备份没成功就不会执行覆盖性拉取。检查磁盘空间与 `backups/github-sync/` 权限 |
| 拉取后出现冲突面板 | 属正常行为（冲突不再被静默偏向本地）。逐文件选择保留本地/远程，全部解决后结果会自动应用到数据目录 |
...
- **备份恢复/删除的路径穿越漏洞** —— `backupId` 未做校验，构造含 `../` 的值可越界删除或覆盖任意目录。
  现在要求严格的秒级时间戳格式，并追加解析后前缀断言，不合法直接返回 400
- **Token 不再明文落盘** —— 迁移到数据目录之外的 `0600` 密钥文件；不再写入 `.git/config` 的 remote URL；
  不再出现在命令行参数中（改用 `GIT_ASKPASS` 助手，凭据经环境变量传递）
- `/extensions` 接口返回的仓库地址会先做凭据脱敏

**重要修复**

- **拉取前自动备份会静默失效** —— 旧实现在备份被关闭时静默返回 `null`（连一条日志都没有），
  备份报错时也只记一条 warning 就继续执行覆盖性拉取。现在：备份**报错直接中止拉取**；
  备份关闭或无可备份数据时写入明确日志，不再静默
- **客户端可能静默关闭自动备份** —— 配置面板加载完成前，任何输入变化都会把尚未回填的复选框状态整体写回，
  导致「拉取前自动备份」被永久关闭。现增加加载完成守卫，复选框默认勾选
- **移除拉取的 `-X ours`** —— 该参数让所有冲突静默偏向本地、丢弃远端更新，
  同时使冲突解决界面永远无法触发。现在冲突会正常暴露给冲突面板
- **推送前预拉取遇到冲突不再被吞掉** —— 旧实现无条件忽略异常，会把仓库留在 merge 状态，
  随后把冲突标记（`<<<<<<<`）当作正常内容提交。现在会中止 merge 并提示先处理冲突
- **冲突解决结果未回写本地** —— 解决完冲突只提交到同步仓库，数据目录从未更新，
  用户看不到任何本地变化。现在全部解决后自动应用
- **增量拷贝失效** —— `copyFileIfChanged` 比较 mtime，但拷贝时未保留时间戳，
  目标 mtime 永远是"刚刚"，导致每次同步都全量重拷。已修正
...
- 若你的 Token 曾以明文形式存在于 `github-data-sync-config.json`，或曾出现在同步仓库的 `.git/config` 中，
  建议到 GitHub 吊销并重新生成一个
